...
namespace LiveCell_Gui
{
    partial class LiveCell
    {
        ...
        const string MOTOR_POS_REQ = "motorposreq";
        const string MOTOR_POS = "motorpos";
        const string MOTOR_POS_XYZ = "motorposxyz";
        ...
        private const int DEFAULT_SPEED_Z = 8000;
...
//#else
        ...
        private void parse_string(string recv)
        {
            ...
            else if (recv.Contains(MOTOR_POS_REQ) || recv.Contains(MOTOR_POS_XYZ))
            {
                if (cbdebug.Checked == true) display_data_RX_textbox('#' + recv + '*');

                this.BeginInvoke((MethodInvoker)(() =>
                {
                    //var match = Regex.Match(recv, $@"motorposreq[:,]([-+]?\d+),([-+]?\d+),([-+]?\d+)");
                    string pattern = $@"(\w+)[:,]([-+]?\d+),([-+]?\d+),([-+]?\d+)";
                    var match = Regex.Match(recv, pattern);

                    if (match.Success)
                    {
                        lbcurposx.Text = match.Groups[2].Value;
                        lbcurposy.Text = match.Groups[3].Value;
                        lbcurposz.Text = match.Groups[4].Value;
                    }
                    else
                        display_data_RX_textbox("Match Fail.");
                }));
            ...
            else if (recv.Contains(MOTOR_STATUS_REQ))
            {
                ...
                try
                {
                        ...
                        int.TryParse(values[2], out z))
                    {
                        ...
                        MotorStatus[0] = (byte)x;
                        MotorStatus[1] = (byte)y;
                        MotorStatus[2] = (byte)z;

                        this.BeginInvoke((MethodInvoker)(() =>
                        {
                            if (x == 1) btMoveXasix.BackColor = Color.LightCyan; else btMoveXasix.BackColor = Color.MistyRose;
                            if (y == 1) btMoveYasix.BackColor = Color.LightCyan; else btMoveYasix.BackColor = Color.MistyRose;
                            if (z == 1) btMoveZasix.BackColor = Color.LightCyan; else btMoveZasix.BackColor = Color.MistyRose;
                        }));
                    }
                    else
                    {
